...
- **Checkout social buttons are relocated, not added.** Nextend renders them from its own EDD *checkout* integration, whose position choices are all whole-form (`edd_checkout_form_top`, `edd_purchase_form_before_email`, `…_before_submit`, `…_after_submit`), so they land above Personal Info rather than by the login form. Its `edd_login` setting *does* offer before/after the login fields, but that hooks `edd_login_fields_after`, which EDD fires only for the standalone login block and shortcode — the checkout fires `edd_checkout_login_fields_after`. So the placement isn't reachable from Nextend's settings; whichever hook it registered is unhooked on `wp_loaded` (safely after Nextend registers during `plugins_loaded`) and the buttons are rendered at the login fieldset instead.
